...
class SeerAgent(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        if not seers:
            # No seer in this game — nothing to do. Still must yield.
            if False:
                yield
            return
        seer = seers[0]
        ...
        if False:
            yield  # keep this an async generator — see callout below
```

> ⚠️ **Common pitfall:** `_run_async_impl` **must be an async generator**. Python decides this by looking for `yield` anywhere in the function body, even unreachable code. Our Seer never has events to emit (it only mutates state), so we add `if False: yield` at the end. Without it, ADK will throw an opaque `TypeError: ... is not an async generator`. This trick will appear in every "no-LLM-events" agent we write.
...
from .players import create_player
from .night import call_player, format_public, parse_single
...
from .night import NightPhaseAgent, DeathResolverAgent
...
| `TypeError: __anext__ ... is not an async generator` | A custom `BaseAgent._run_async_impl` has no `yield`. | Add `if False: yield` at the bottom. |
